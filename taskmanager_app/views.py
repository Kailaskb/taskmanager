...
from .serializers import TaskConfigSerializer, BitMapSerializer, MapBackupModelsSerializer
...
class MapBackupModelsView(APIView):
    def post(self, request):
        serializer = MapBackupModelsSerializer(data=request.data)
        if serializer.is_valid():
            obj = serializer.save()
            data = MapBackupModelsSerializer(obj).data
            return Response(data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

    def get(self, request):
        objects = MapBackupModels.objects.all()
        data = MapBackupModelsSerializer(objects, many=True).data
        return Response(data, status=status.HTTP_200_OK)

    def put(self, request, id):
        obj = get_object_or_404(MapBackupModels, id=id)
        serializer = MapBackupModelsSerializer(obj, data=request.data, partial=True)
